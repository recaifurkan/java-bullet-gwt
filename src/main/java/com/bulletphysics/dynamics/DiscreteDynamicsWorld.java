		...
		if (getDebugDrawer() != null && (getDebugDrawer().getDebugMode()
			...
			for (i = 0; i < vehicles.size(); i++) {
				for (int v = 0; v < vehicles.getQuick(i).getNumWheels(); v++) {
					...
					axle.set(
							MatrixUtil.getElement(vehicles.getQuick(i).getWheelInfo(v).worldTransform.basis, 0,
									vehicles.getQuick(i).getRightAxis()),
							MatrixUtil.getElement(vehicles.getQuick(i).getWheelInfo(v).worldTransform.basis, 1,
									vehicles.getQuick(i).getRightAxis()),
							MatrixUtil.getElement(vehicles.getQuick(i).getWheelInfo(v).worldTransform.basis, 2,
									vehicles.getQuick(i).getRightAxis()));
	...
	private static class InplaceSolverIslandCallback extends SimulationIslandManager.IslandCallback {
		...
		public void processIsland(ObjectArrayList<CollisionObject> bodies, int numBodies,
			...
			if (islandId < 0) {
				...
				// passed into the solver regardless the
				// island id
	...
	public void debugDrawObject (Transform worldTransform, CollisionShape shape, Vector3 color) {
	 ...
	 else
	 {
		 ...
		 case BOX_SHAPE_PROXYTYPE:
	     {
	         ...
	         Vector3 nHalfExtents = stack.allocVector3();
	         nHalfExtents.set(-halfExtents.x,-halfExtents.y,-halfExtents.z);
	         getDebugDrawer().drawBox(nHalfExtents,halfExtents,worldTransform,color);
...
//		 childTransform.origin = tmp.set(0,halfHeight,0).add(worldTransform.origin);
...
//		 Vector3 start = worldTransform.origin;
//		 
//		 getDebugDrawer().drawLine(start.add(tmp.set(-radius,halfHeight,0).mul(worldTransform.basis)) ,
//				 start.add(tmp.set(-radius,-halfHeight,0)).mul(worldTransform.basis),
//				 color);
//		 getDebugDrawer().drawLine(start.add(tmp.set(radius,halfHeight,0).mul(worldTransform.basis)) ,
//				 start.add(tmp.set(radius,-halfHeight,0)).mul(worldTransform.basis),
//				 color);
//		 getDebugDrawer().drawLine(start.add(tmp.set(0,halfHeight,-radius).mul(worldTransform.basis)) ,
//				 start.add(tmp.set(0,-halfHeight,-radius)).mul(worldTransform.basis),
//				 color);
//		 getDebugDrawer().drawLine(start.add(tmp.set(0,halfHeight,radius).mul(worldTransform.basis)) ,
//				 start.add(tmp.set(0,-halfHeight,radius)).mul(worldTransform.basis),
//				 color);
...
//		 {
//		 MultiSphereShape multiSphereShape = (shape);
//
...
// private BDebugDraw debugDrawer;
// private final Vector3 color = new Vector3();
// private final Transform worldTrans = new Transform();
//
// public DebugDrawcallback(BDebugDraw debugDrawer, Transform worldTrans, Vector3 color) {
// this.debugDrawer = debugDrawer;
...
// }
	...
	private static class ClosestNotMeConvexResultCallback extends ClosestConvexResultCallback {
		...
		public ClosestNotMeConvexResultCallback(CollisionObject me, Vector3 fromA, Vector3 toA,
